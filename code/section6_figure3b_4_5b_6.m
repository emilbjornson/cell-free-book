...
SE_Genie_PRZF_DCC = zeros(K,nbrOfSetups); %P-RZF (Genie-aided)
...
SE_Genie_LPMMSE_DCC = zeros(K,nbrOfSetups); %LP-MMSE (Genie-aided)
SE_Genie_MR_DCC = zeros(K,nbrOfSetups);  %MR (Genie-aided)
...
for n = 1:nbrOfSetups
    ...
    SE_Genie_PRZF_DCC(:,n) = Gen_SE_P_RZF;
    SE_Genie_LPMMSE_DCC(:,n) = Gen_SE_LP_MMSE;
    SE_Genie_MR_DCC(:,n) = Gen_SE_MR;
...
plot(sort(SE_PRZF_DCC(:)),linspace(0,1,K*nbrOfSetups),'k:','LineWidth',2);
plot(sort(SE_Genie_PRZF_DCC(:)),linspace(0,1,K*nbrOfSetups),'b--','LineWidth',2);

xlabel('Spectral efficiency [bit/s/Hz]','Interpreter','Latex');
ylabel('CDF','Interpreter','Latex');
legend({'P-MMSE','P-MMSE (Genie-aided)','P-RZF','P-RZF (Genie-aided)'},'Interpreter','Latex','Location','SouthEast');
xlim([0 12]);
...
plot(sort(SE_LPMMSE_DCC(:)),linspace(0,1,K*nbrOfSetups),'k-','LineWidth',2);
plot(sort(SE_Genie_LPMMSE_DCC(:)),linspace(0,1,K*nbrOfSetups),'r-.','LineWidth',2);
plot(sort(SE_MR_DCC(:)),linspace(0,1,K*nbrOfSetups),'k:','LineWidth',2);
plot(sort(SE_Genie_MR_DCC(:)),linspace(0,1,K*nbrOfSetups),'b--','LineWidth',2);

xlabel('Spectral efficiency [bit/s/Hz]','Interpreter','Latex');
ylabel('CDF','Interpreter','Latex');
legend({'LP-MMSE','LP-MMSE (Genie-aided)','MR','MR (Genie-aided)'},'Interpreter','Latex','Location','SouthEast');
xlim([0 12]);